...
with st.form("master_form"):
    ...
    with col1:
        st.markdown("##### 📝 基本資料")
        name = st.text_input("🏠 物件名稱")
        location = st.text_input("📍 鄰近地點/商圈")
        price_raw = st.text_input("💰 總價 (打數字即可)")
        ping_raw = st.text_input("📐 建坪 (打數字即可)")
        land_ping_raw = st.text_input("🌲 地坪 (打數字即可)")
    with col2:
        st.markdown("##### 📏 規格細節")
        layout_raw = st.text_input("🚪 格局 (打3個數字，如322)")
        floor_raw = st.text_input("🏢 樓層 (如5/12或4)")
        age_raw = st.text_input("📅 屋齡 (打數字即可)")
        parking_raw = st.text_input("🚗 車位")
        features = st.text_area("✨ 物件特色", height=68)
    with col3:
        st.markdown("##### 📸 發佈設定")
        link = st.text_input("🔗 官網詳情連結")
        uploaded_files = st.file_uploader("📸 上傳物件照片", type=['jpg','png','jpeg'], accept_multiple_files=True)
